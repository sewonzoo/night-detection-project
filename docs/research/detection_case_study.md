...
RT-DETRv2는 DETR 계열의 실시간 객체 탐지 모델로, NMS(Non-Maximum Suppression) 없이 end-to-end로 객체를 탐지한다. 
...
`γ=3.0` 감마 보정으로 픽셀값이 강하게 감쇠하여 이미지가 매우 어두워졌다. 그 결과 두 가지 현상이 동시에 발생한다:
...
- 객체 크기별로는 **small object의 mAP가 가장 낮음** (원본 31.7% → 야간 20.9% → 복원 18.6%)
