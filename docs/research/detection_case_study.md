...
```java
■ 사용 모델: RT-DETRv2 (2024.07, Baidu)

▸ 왜 RT-DETRv2인가?

  1. Transformer 기반 → 전역적 특징 추출
     · CNN: 지역적 특징 / Transformer: 전역적 맥락
     · 야간 이미지처럼 부분 정보 손실 환경에 유리
  
  2. NMS-free end-to-end 구조
     · YOLO: 박스 다수 → NMS 정리 (후처리 의존)
     · RT-DETRv2: 처음부터 필요한 만큼만 출력
     · → 입력 품질 비교 실험에 적합 (후처리 변수 제거)
  
  3. Distinct Sampling Points
     · 객체 크기(scale)에 따라 적응적 샘플링
     · 작은 객체엔 많이, 큰 객체엔 적게
     · -> 효율적인 계산 가능
  
  4. Hybrid Encoder (CNN + Transformer)
     · CNN의 속도 + Transformer의 전역 표현력
     · → 실시간성 유지
```

"본 실험에서는 2024년 7월 발표된 최신 객체 탐지 모델 RT-DETRv2를 사용했습니다. 선택 이유는 다음 네 가지입니다.

**첫째, Transformer 기반의 전역적 특징 추출 능력입니다.** CNN은 지역적 특징(local feature)에 강점이 있지만, Transformer는 이미지 전체의 맥락을 보는 전역적 특징(global feature)에 강합니다. 야간 이미지처럼 부분적으로 어두워 지역 정보가 손실된 환경에서는, 전역 정보를 활용해 객체를 추론할 수 있는 Transformer 기반 모델이 더 유리합니다.

**둘째, NMS-free end-to-end 구조입니다.** YOLO 계열은 다수의 박스 후보를 뿌린 뒤 NMS(Non-Maximum Suppression)로 정리하는 방식이라, 후처리 단계에 의존성이 큽니다. 반면 RT-DETRv2는 처음부터 객체 수에 맞춰 박스를 출력하는 end-to-end 구조라 후처리 일관성이 보장됩니다. 이는 '입력 이미지 품질만이 결과 차이를 만드는지' 비교하는 본 실험의 통제된 환경에 적합합니다.

**셋째, Distinct Sampling Points를 통한 스케일 적응 능력입니다.** RT-DETRv2는 Deformable Attention에서 객체 크기에 따라 다른 수의 sampling point를 사용할 수 있습니다. 큰 객체는 적게, 작은 객체는 많이 샘플링하여 다양한 크기의 객체를 효율적으로 처리합니다.

**넷째, 하이브리드 인코더로 실시간성 유지입니다.** RT-DETRv2는 CNN의 빠른 지역 특징 추출과 Transformer의 전역 표현 능력을 결합한 하이브리드 인코더를 사용합니다. 그 결과 COCO mAP 53.4%를 달성하며 YOLOv8 시리즈와 경쟁하는 SOTA 성능을, 실시간 속도(T4 GPU 108 FPS)로 동시에 달성했습니다.

정리하면, **구형 모델이 아닌 2024년 최신 모델 RT-DETRv2를 선택한 이유는, 실시간성을 유지하면서도 전역적 특징 추출 능력이 뛰어나 저조도 환경에서의 객체 인식에 유리하기 때문**입니다."
...
| 조건 | mAP@0.5 | mAP@0.5:0.95 | mAP small | mAP medium | mAP large |
| --- | --- | --- | --- | --- | --- |
| **A: Original** | **68.43%** | **53.37%** | 31.7% | 55.4% | 75.6% |
| **B: Low-light (γ=3.0)** | 53.79% | 40.81% | 20.9% | 41.5% | 62.2% |
| **C: Enhanced (RetinexFormer)** | 49.84% | 36.90% | 18.6% | 40.3% | 57.5% |

**실험 결과:**
원본 sample_200에서 측정한 mAP@0.5:0.95는 53.37%로, 논문 공식 COCO 점수 53.4%와 거의 일치합니다. 이는 모델 로딩과 추론 환경이 정상 작동함을 정량적으로 입증한 것입니다.

**RT-DETRv2 자체의 한계 (논문 기준)**
- DETR 계열 공통: **작은 객체 탐지가 상대적으로 약함**
- 우리 실험 결과도 일치: small mAP 31.7% < large mAP 75.6%
- 야간 합성에서 small mAP가 가장 크게 떨어짐(31.7 → 20.9 → 18.6%)
- 이는 RT-DETRv2의 일반적 특성이 우리 실험에서도 재현됨을 보여줌
